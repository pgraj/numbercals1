...
@register(
    ...
    summary=(
        ...
        "angle, with a triangle diagram showing the equal ratios."
    ),
...
def compute(mode="side", known_side=None, known_angle=None,
            other_angle=None, other_side=None):
    try:
        ...
        if mode == "side":
            a = num(known_side)       # side opposite known_angle (A)
            A = num(known_angle)
            B = num(other_angle)      # angle opposite the side we want (b)
            if a is None or A is None or B is None:
                return _err("Enter a side, its opposite angle, and the angle opposite the side you want.")
            if not (0 < A < 180) or not (0 < B < 180):
                return _err("Angles must be between 0° and 180°.")
            if a <= 0:
            ...
            steps = [
                ...
                {"label": "Rearrange for the unknown side",
                 "math": r"\(b = \dfrac{a\,\sin B}{\sin A} = \dfrac{%s\,\sin %s^\circ}{\sin %s^\circ}\)"
                         % (_fmt(a), _fmt(B), _fmt(A)),
                 "note": "a = %s opposite A = %s°; we want b opposite B = %s°." % (_fmt(a), _fmt(A), _fmt(B))},
                {"label": "Evaluate",
                 "math": r"\(b = %s\)" % _fmt(b),
            ...
            a = num(known_side)       # side opposite known_angle A
            A = num(known_angle)
            b = num(other_side)       # side opposite the angle we want (B)
            if a is None or A is None or b is None:
                return _err("Enter two sides and the angle opposite one of them.")
            if not (0 < A < 180):
                return _err("The angle must be between 0° and 180°.")
            if a <= 0 or b <= 0:
            ...
            steps = [
                ...
                {"label": "Make sin B the subject",
                 "math": r"\(\sin B = \dfrac{b\,\sin A}{a} = \dfrac{%s\,\sin %s^\circ}{%s}\)"
                         % (_fmt(b), _fmt(A), _fmt(a)),
                 "note": "Then take the inverse sine."},
                {"label": "Evaluate the angle",
                 "math": r"\(B = \sin^{-1}(%s) \approx %s^\circ\)" % (_fmt(ratio), _fmt(B)),
                 "note": "Watch for the ambiguous (SSA) case — a second obtuse angle 180°−B may also fit."},
            ]
            return {
                "result": _fmt(B) + "\u00b0", "mode": "angle", "value": round(B, 6),
                "a": a, "A": A, "b": b, "B": round(B, 6),
