...
triangles are possible, and give the angle(s).
"""
import math
...
@register(
    ...
    summary=(
        ...
        "may give no triangle, one triangle, or two different triangles, with both "
        "valid triangles drawn side by side and an explanation of which case "
        "applies and why."
    ),
...
def compute(a=None, b=None, A=None):
    ...
    try:
        ...
        a_, b_, A_ = num(a), num(b), num(A)
        if a_ is None or b_ is None or A_ is None:
            return _err("Enter side a (opposite angle A), side b, and angle A.")
        if a_ <= 0 or b_ <= 0:
            return _err("Side lengths must be greater than zero.")
        if not (0 < A_ < 180):
            return _err("Angle A must be between 0° and 180°.")

        sinB = b_ * math.sin(math.radians(A_)) / a_
        steps = [
            {"label": "Apply the sine rule for sin B",
             "math": r"\(\sin B = \dfrac{b\,\sin A}{a} = \dfrac{%s\,\sin %s^\circ}{%s} = %s\)"
                     % (_fmt(b_), _fmt(A_), _fmt(a_), _fmt(sinB)),
        ...
        B2 = 180 - B1
        # Second triangle valid only if A + B2 < 180 and B2 distinct.
        ...
        if two:
            ...
            steps.append({"label": "Two possible angles",
                          "math": r"\(B = %s^\circ \text{ or } B = 180^\circ - %s^\circ = %s^\circ\)"
                                  % (_fmt(B1), _fmt(B1), _fmt(B2)),
                          "note": "Both keep the angle sum under 180°, so two distinct triangles fit."})
            steps.append({"label": "Remaining angle in each",
                          "math": r"\(C = %s^\circ \text{ or } %s^\circ\)" % (_fmt(C1), _fmt(C2)),
                          "note": "Each choice of B gives a different third angle and triangle."})
            return {"result": "Two triangles (B = %s° or %s°)" % (_fmt(B1), _fmt(B2)),
                    "count": 2, "a": a_, "b": b_, "A": A_,
            ...
            steps.append({"label": "One triangle",
                          "math": r"\(B = %s^\circ,\ C = %s^\circ\)" % (_fmt(B1), _fmt(C1)),
                          "note": "The obtuse alternative would push the angle sum past 180°, so only one triangle works."})
            return {"result": "One triangle (B = %s°)" % _fmt(B1),
                    "count": 1, "a": a_, "b": b_, "A": A_,
